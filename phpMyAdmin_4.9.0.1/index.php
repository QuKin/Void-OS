<?php
$token = isset($_GET['phpstudy_token'])?$_GET['phpstudy_token']:false;
if(!$token){
 $token = isset($_COOKIE["phpstudy_token"])?$_COOKIE["phpstudy_token"]:false;
}
if(!$token){
 header("HTTP/1.1 404 Not Found");exit;
}
$location_token = '93C4E5';
$location_token = str_replace("\n",'',$location_token);
if($token != $location_token){
 header("HTTP/1.1 404 Not Found");exit;
}
setcookie("phpstudy_token",$token);
